...
import { Envelope, Lock, ArrowRight, MusicNoteBeamed } from 'react-bootstrap-icons';

const Login = ({ setActiveTab, onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validar el inicio de sesión (simulación)
    if (email === 'test@example.com' && password === '123456') {
       // Pasar los datos del usuario al componente padre
      onLogin({
        email: email,
        name: 'Usuario de Prueba',
        // Agregar más datos del usuario según necesites
      });
    } else {
      setShowAlert(true);
    }
            ...
            <Card className="shadow-lg border-0 rounded-4 overflow-hidden" style={{
              ...
              <div style={{
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  <MusicNoteBeamed className="mb-2" style={{ fontSize: '2rem', color: '#e8d7b0' }} />
                  <h3 className="fw-bold mb-1" style={{ color: '#e8d7b0', textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>Login</h3>
                  <p className="mb-0" style={{ color: '#c9b6a2' }}>Enter your login credentials</p>
                </div>
              </div>
              ...
              <Card.Body className="p-4 p-md-5" style={{ background: 'rgba(26, 11, 46, 0.7)' }}>
                {showAlert && (
                  <Alert variant="danger" onClose={() => setShowAlert(false)} dismissible 
                    ...
                    Incorrect credentials - Please verify your information
                  </Alert>
                )}
                ...
                <Form onSubmit={handleSubmit}>
                  ...
                  <Form.Group className="mb-4">
                    <Form.Label className="small fw-bold" style={{ color: '#d4c19c' }}>Password</Form.Label>
                    <div className="input-group" style={{ 
                      border: '1px solid #5a3a8a',
                      borderRadius: '8px',
                      background: 'rgba(90, 58, 138, 0.2)'
                  ...
                  >
                    {isLoading ? 'Starting...' : (
                      <>
                        Log in <ArrowRight className="ms-2" />
                      </>
                    )}
                  </Button>
                </Form>
              ...
              <div className="py-3 text-center" style={{ 
                ...
                <p className="mb-0 small" style={{ color: '#c9b6a2' }}>
                  ...
                  <Button 
                    variant="link" 
                    className="text-decoration-none p-0 small fw-bold" 
